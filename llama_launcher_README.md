...
- **設定持久化**：支援儲存/重置所有路徑預設值（Model / Llama / Log Dir），重啟後自動套用。
- **WSL/LAN 網路模式**：啟用後綁定 `0.0.0.0`，讓 WSL 或區網裝置可通行 `host.docker.internal` 連線。
- **OpenClaw 端口自動補全**：切換分頁時自動偵測 llama.cpp 端口並填入配置。
- **mmproj 手動瀏覽強化**：支援 `.gguf` / `.bin` 雙格式，切換模型時保留手動選擇不覆蓋。
